dnl
dnl check for GCC 6.x with plugin support enabled and plugins
dnl
...
echo xi386_gcc6 checking "$GCC_TO_BE_CHECKED"
for compiler in $GCC_TO_BE_CHECKED; do
   ...
   echo xi386_gcc6 version is "$I386_GCC6_VERSION"
      ...
      AS_VERSION_COMPARE($I386_GCC6_VERSION, $MIN_GCC6, echo "xi386_g1 checking $compiler >= $MIN_GCC6... no"; min=no, echo "xi386_g2 checking $compiler >= $MIN_GCC6... yes"; min=yes, echo "xi386_g3 checking $compiler >= $MIN_GCC6... yes"; min=yes)
      if test "$min" = "no" ; then
         continue;
      fi
      AS_VERSION_COMPARE($I386_GCC6_VERSION, $MAX_GCC6, echo "xi386_g4 checking $compiler < $MAX_GCC6... yes"; max=yes, echo "xi386_g5 checking $compiler < $MAX_GCC6... no"; max=no, echo "xi386_g6 checking $compiler < $MAX_GCC6... no"; max=no)
      if test "$max" = "no" ; then
         continue;
      fi
